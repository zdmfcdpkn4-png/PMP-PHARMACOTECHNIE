...
const { Hero, Marquee, Discographie, Concerts, Membres, Boutique, Pharmacobar, Galerie, Paroles, Timeline, FanMap, Contact } = window.PmpSections;
...
/* ───── Scroll lights — colored beams that follow scroll (throttled) ── */
function ScrollLights() {
  ...
  React.useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        const p = sy / max;
        // Throttle DOM writes: only update if p changed meaningfully (>0.5%).
        const prev = parseFloat(ref.current.dataset.p || "0");
        if (Math.abs(prev - p) < 0.005) return;
        ref.current.dataset.p = p.toFixed(3);
        ref.current.style.setProperty("--sl-p", p.toFixed(3));
        ref.current.style.setProperty("--sl-h1", `${(p * 360) % 360}deg`);
        ref.current.style.setProperty("--sl-h2", `${(p * 360 + 80) % 360}deg`);
        ref.current.style.setProperty("--sl-h3", `${(p * 360 + 200) % 360}deg`);
      });
    };
  ...
  return (
    <div ref={ref} className="scroll-lights" aria-hidden>
      <span className="sl-beam sl-beam-1"></span>
      <span className="sl-beam sl-beam-2"></span>
      <span className="sl-beam sl-beam-3"></span>
    </div>
  );
}
function PmpPlayer({ playingN, onStop }) {
  const tracks = window.PMP.PMP_TRACKS;
  const track = playingN ? tracks.find(t => t.n === playingN) : null;
  const [vizBars, setVizBars] = React.useState(() => new Array(18).fill(4));
  ...
  const next = () => {
    const idx = tracks.findIndex(t => t.n === track.n);
    const nx = tracks[(idx + 1) % tracks.length];
    PmpAudio.start(nx);
  };
  const prev = () => {
    const idx = tracks.findIndex(t => t.n === track.n);
    const px = tracks[(idx - 1 + tracks.length) % tracks.length];
    PmpAudio.start(px);
  };

  return (
    <aside className={`pmp-player ${collapsed ? "collapsed" : ""}`}>
      {/* spinning vinyl thumbnail */}
      <div className="pp-vinyl" onClick={() => setCollapsed(!collapsed)} aria-label="Réduire">
        <svg viewBox="0 0 100 100" className="pp-disc">
          <defs>
            <radialGradient id="pp-vinyl-g" cx="35%" cy="30%" r="80%">
              <stop offset="0%" stopColor="#3a3a3a"/>
              <stop offset="60%" stopColor="#0a0a0a"/>
              <stop offset="100%" stopColor="#000"/>
            </radialGradient>
          </defs>
          <circle cx="50" cy="50" r="50" fill="url(#pp-vinyl-g)"/>
          {[45, 39, 33, 27, 21].map(r => (
            <circle key={r} cx="50" cy="50" r={r}
                    fill="none" stroke="rgba(255,255,255,.06)" strokeWidth=".6"/>
          ))}
          <circle cx="50" cy="50" r="18" fill="var(--c-pink)"/>
          <text x="50" y="54" textAnchor="middle"
                fontFamily="Metal Mania, serif" fontSize="14" fill="#fff">PMP</text>
      ...
      {!collapsed && (
        <div className="pp-body">
          <div className="pp-meta">
            <span className="pp-rx">℞ EN COURS</span>
            <h4 className="pp-title">{track.title}</h4>
            <span className="pp-sub">{track.sub}</span>
          </div>
          <div className="pp-viz" aria-hidden>
            {vizBars.map((h, i) => (
          ...
          <div className="pp-controls">
            <button onClick={prev} aria-label="Précédent">
              <svg width="14" height="14" viewBox="0 0 14 14"><polygon points="12,2 4,7 12,12" fill="currentColor"/><rect x="2" y="2" width="2" height="10" fill="currentColor"/></svg>
            </button>
            <button className="pp-stop" onClick={onStop} aria-label="Stop">
              <svg width="14" height="14" viewBox="0 0 14 14"><rect x="3" y="3" width="8" height="8" fill="currentColor"/></svg>
            </button>
            <button onClick={next} aria-label="Suivant">
              <svg width="14" height="14" viewBox="0 0 14 14"><polygon points="2,2 10,7 2,12" fill="currentColor"/><rect x="10" y="2" width="2" height="10" fill="currentColor"/></svg>
            </button>
          </div>
        </div>
      )}
...
function Nav({ cartCount, onOpenCart }) {
  const links = [
    ...
    { id: "fanmap",       label: "Fans",         icon: "fans"     },
...
function App() {
  ...
  return (
    <>
      ...
      <PmpPlayer playingN={playingN} onStop={() => PmpAudio.stop()} />
