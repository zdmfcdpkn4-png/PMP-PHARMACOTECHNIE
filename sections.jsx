...
function ProductPic({ product }) {
  ...
  switch (product.id) {
    ...
    case "plectre": {
      const picks = [
        { x: 56,  y: 60,  r: -16, col: "var(--c-pink)"    },
        { x: 132, y: 56,  r: 22,  col: "var(--c-orange)"  },
        { x: 38,  y: 124, r: -32, col: "var(--c-mustard)" },
        { x: 108, y: 132, r: 6,   col: "var(--c-lilac)"   },
        { x: 160, y: 116, r: 36,  col: "var(--c-pink-2)"  },
      ];
      return (
        <svg viewBox="0 0 200 200">
          {studioDefs("pk")}
          ...
          {picks.map((p, i) => (
            <g key={i} transform={`translate(${p.x} ${p.y}) rotate(${p.r})`}>
              <ellipse cx="3" cy="6" rx="22" ry="26" fill="rgba(0,0,0,.32)"/>
              {/* pick shape: rounded triangle */}
              <path d="M -22 -20 Q 0 -28 22 -20 Q 22 -8 0 28 Q -22 -8 -22 -20 Z"
                    fill={p.col}/>
              {/* highlight */}
              <path d="M -18 -16 Q -2 -22 12 -18 Q 6 -8 -8 0 Q -16 -6 -18 -16 Z"
                    fill="rgba(255,255,255,.45)"/>
              {/* tiny stamped logo */}
              <image href={HERO} x="-14" y="-12" width="28" height="28"
                     preserveAspectRatio="xMidYMid meet" style={{ opacity: .85 }}/>
              {/* edge highlight */}
              <path d="M -22 -20 Q 0 -28 22 -20" fill="none" stroke="rgba(255,255,255,.45)" strokeWidth=".8"/>
            </g>
          ))}
        </svg>
      );
...
function CocktailScene({ cocktail }) {
  ...
  return (
    <svg viewBox="0 0 200 260" preserveAspectRatio="xMidYMid meet" width="100%" height="100%">
      ...
      {/* big floating cocktail emoji to anchor identity */}
      <text x="180" y="240" textAnchor="end" fontSize="22">{c.emoji}</text>

      {/* Caption tab */}
      <rect x="12" y="20" width="74" height="20" fill="rgba(0,0,0,.65)" rx="2"/>
      <text x="20" y="34" fontFamily="JetBrains Mono, monospace" fontSize="9"
            fill="white" letterSpacing="2" fontWeight="700">℞ {c.abv}° · {c.glass.slice(0,4).toUpperCase()}</text>
    </svg>
  );
}
