...
function FanMap() {
  ...
  return (
    <section className="pmp fanmap-section" id="fanmap" data-screen-label="10 Fans">
      ...
      <p className="section-sub">
        L'armée de fans PMP s'étend sur toute la planète. Total recensé : <b style={{ color: "var(--accent)" }}>{total.toLocaleString("fr-FR")}</b> fans · compteur live mis à jour à chaque écoute.
      </p>
      ...
      <div className="fanmap-wrap fanmap-img">
        <img src="assets/fanmap-monde.png" alt="Carte mondiale des fans de PMP — Pharmacotechnie est partout, rejoignez la Légion"
             className="fanmap-photo" loading="lazy" draggable="false"/>
        <div className="fanmap-overlay" aria-hidden>
          {[
            { x: 19, y: 52, hub: false },  // North America
            { x: 23, y: 56, hub: false },
            { x: 31, y: 70, hub: false },  // South America
            { x: 34, y: 64, hub: false },
            { x: 52, y: 42, hub: true  },  // France / Europe (hub)
            { x: 55, y: 46, hub: false },
            { x: 53, y: 58, hub: false },  // Africa
            { x: 68, y: 46, hub: false },  // Asia / Russia
            { x: 74, y: 50, hub: false },
            { x: 67, y: 60, hub: false },  // India
            { x: 80, y: 60, hub: false },  // SE Asia
            { x: 84, y: 73, hub: false },  // Australia
          ].map((p, i) => (
            <span key={i}
                  className={`fm-glow ${p.hub ? "fm-glow-hub" : ""}`}
                  style={{ left: p.x + "%", top: p.y + "%", animationDelay: `${(i % 5) * 0.4}s` }}>
              <span className="fm-glow-pulse"></span>
              <span className="fm-glow-core"></span>
            </span>
          ))}
        </div>
      </div>
